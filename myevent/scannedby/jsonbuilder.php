...
function getDataScannedBy($id,&$link){
   ...
   /* $a = array();
	...
	return $a;
	*/
	$query = "SELECT actions.account_ID FROM tags, invitations, actions WHERE tags.type='P' AND invitations.tag_ID=tags.tag_ID AND actions.tag_ID=tags.tag_ID AND invitations.account_ID=".$id." AND actions.type='P' GROUP BY actions.account_ID";
...
function buildJSON($id,&$link){
    $data = getDataScannedBy($id,$link);
    $json = addslashes(json_encode($data));
    $fileHandler = fopen("/var/www/vhosts/smart-qr.com/subdomains/grand/html/myevent/scannedby/static/grand/json".$id, 'w');  //accountID FROM DATABASE
...
$query = "SELECT invitations.account_ID FROM tags, invitations, actions WHERE tags.type='P' AND invitations.tag_ID=tags.tag_ID AND actions.tag_ID=tags.tag_ID AND actions.type='P' GROUP BY invitations.account_ID";
@mysqli_real_query($link,$query);
$result = mysqli_store_result($link);
if(mysqli_num_rows($result)>0)
{
