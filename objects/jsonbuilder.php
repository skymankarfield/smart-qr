...
function getDataListObjects($id,&$link){
	...
	*/
	...
	$query = "SELECT * FROM invitations, tags WHERE invitations.account_ID=".$id." AND invitations.event_ID=".$_SESSION['eventID']." AND invitations.tag_ID=tags.tag_ID AND tags.type='O' AND invitations.status='1'";
...
function buildJSONListObjects($id,&$link){
    ...
    $fileHandler = fopen("./static/".$_SESSION['event']."/list".$id, 'w');
...
function buildJSONPrivateObject($id,&$link){
    ...
    $json = addslashes(json_encode($data));
    $fileHandler = fopen("./static/".$_SESSION['event']."/private".$id, 'w');
    fwrite($fileHandler, $json);
    fclose($fileHandler);
}
?>
