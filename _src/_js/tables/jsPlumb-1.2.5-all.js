 ...
 */
function addConnection(node1, node2, text, size, color){
  ...
  if(typeof connections[node1][node2] == 'undefined' && typeof connections[node2][node1] == 'undefined'){
    var _makeOverlay = function() { 
        return new jsPlumb.Overlays.Arrow({foldback:0.7, fillStyle:"rgba(0,0,0,0.75)", location:0.5, width:25}); 
    };
    var connection = jsPlumb.connect({
                source: node1, 
                target: node2, 
                anchors:["AutoDefault", "AutoDefault"],
                connector:new jsPlumb.Connectors.Bezier(120),
                backgroundPaintStyle:{lineWidth:size + 3,strokeStyle: "rgba(0,0,0,0.75)"},  
                paintStyle:{lineWidth: size ,strokeStyle: color},
 ...
 */
function getNumberOfConnections(node){
  var counter = 0;
...
(function() {
	...
	var jsPlumbInstance = function() {
		...
		var _initDraggableIfNecessary = function(element, isDraggable, dragOptions) {
			...
			if (true) {
				if (true) { // Pretty much a hack to force the element to become draggable
		...
		var Endpoint = function(params) {
			...
			this.paint = function(params) {
				...
				if (!timestamp || self.timestamp !== timestamp) {
					...
					//Do not draw the endpoint, since the relation browser is not using them.
		...
		this.connect = function(params) {
			...
			// fire an event
			_currentInstance.fireUpdate("jsPlumbConnection", {
				source : jpc.source, target : jpc.target, canvas: jpc.canvas,
				sourceId : jpc.sourceId, targetId : jpc.targetId,
				sourceEndpoint : jpc.endpoints[0], targetEndpoint : jpc.endpoints[1]
			});
			// force a paint
    ...
    */
	...
	jsPlumb.Endpoints.Rectangle = function(params) {
    	...
    	this.paint = function(anchorPoint, orientation, canvas, endpointStyle, connectorPaintStyle) {    		
			...
			var ctx = canvas.getContext('2d');initDraggable
			var style = jsPlumb.extend({}, endpointStyle);
			if (style.fillStyle ==  null) style.fillStyle = connectorPaintStyle.strokeStyle;
			jsPlumb.extend(ctx, style);
...
(function($) {	    
	jsPlumb.CurrentLibrary = {	        
      ...
      var width = $("#graph").width() - 2;
      var offset = jsPlumb.CurrentLibrary.getOffset(el);
      var minWidth = Math.min(offset.left, width - el.width());
			jsPlumb.CurrentLibrary.setOffset(el, {left: minWidth, top: offset.top});
